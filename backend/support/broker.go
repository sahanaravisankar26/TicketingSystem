...
func NewBroker() *Broker {
	return (*Broker)(&constants.Broker{
		ActiveTickets: make(map[string]constants.Issue),
		Admins:        make(map[chan constants.TicketEvent]bool),
		Users:         make(map[string]map[chan constants.TicketEvent]bool),
	})
}

func (b *Broker) UpdateInternalState(event constants.TicketEvent) {
	b.Mu.Lock()

	// Update the Map (The "Snapshot")
	if event.Action == constants.DELETE {
		delete(b.ActiveTickets, event.Ticket.Id)
	} else {
		// This handles CREATE and UPDATE identically (Upsert)
		b.ActiveTickets[event.Ticket.Id] = event.Ticket
	}
	b.Mu.Unlock()

	// Now broadcast the change to all connected admins
	b.Mu.RLock()
	defer b.Mu.RUnlock()
	for clientChan := range b.Admins {
		clientChan <- event
	}
}
...
func (b *Broker) Broadcast(action string, ticket constants.Issue) {
	b.Mu.Lock()
	defer b.Mu.Unlock()
	event := constants.TicketEvent{Action: constants.Action(action), Ticket: ticket}
...
func (b *Broker) ServeAdminSSE(cluster *gocb.Cluster) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		b.Mu.RLock()
		for _, ticket := range b.ActiveTickets {
			msg, _ := json.Marshal(constants.TicketEvent{
...
func (b *Broker) Bootstrap(cluster *gocb.Cluster) error {
    query := "SELECT id, email, issue, description, message, state FROM `tickets`._default._default"
    res, err := cluster.Query(query, nil)
    if err != nil {
        return err
    }
    ...
    for res.Next() {
        var t constants.Issue
        if err := res.Row(&t); err == nil {
            b.ActiveTickets[t.Id] = t // Load memory map
        }
    }
    return nil
}
